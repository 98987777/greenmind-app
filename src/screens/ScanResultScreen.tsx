...
const ScanResultScreen = () => {
  ...
  useEffect(() => {
    ...
    // ---- 1) Fetch uploaded image from Storage ----
    (async () => {
      try {
        ...
        if (!found) {
          Alert.alert(
            'Image not found',
            'We couldn’t locate the uploaded image in Storage. Please re-scan.'
          );
        }
        setImageUrl(found);
      } catch (err: any) {
        ...
        Alert.alert(
          'Storage Error',
          err?.message || 'Failed to retrieve image from Storage.'
        );
      }
    })();
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        ...
        Alert.alert(
          'Firestore Error',
          err?.message || 'Failed to read analysis result.'
        );
      }
    );
  ...
  const normalizeAndSet = (data: any) => {
    const steps = Array.isArray(data?.recyclingSteps)
      ? data.recyclingSteps.map(String)
      : [];
  ...
  const handleRecycle = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Error',
        error?.message || 'There was a problem saving your scan.'
      );
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {aiResult.name === 'Unknown' ? (
          <View
            style={[
              styles.card,
              { backgroundColor: '#FFF8E1', borderColor: '#FFB300', borderWidth: 1 },
            ]}
          >
            <Text style={[styles.cardTitle, { color: '#FF6F00' }]}>⚠️ Analysis Failed</Text>
            ...
            {aiResult.recyclingSteps?.length > 0 && (
              <Text style={{ fontSize: 14, fontStyle: 'italic', color: '#8A8A8E' }}>
...
const styles = StyleSheet.create({
  ...
  loadingText: { marginBottom: 10, fontSize: 16, color: '#8A8A8E' },
  dotsContainer: { flexDirection: 'row', justifyContent: 'center', marginTop: 10 },
  dot: { width: 12, height: 12, borderRadius: 6, backgroundColor: '#00C851', marginHorizontal: 6 },
...
infoRow: { 
  ...
  alignItems: 'flex-start',   // allow text to wrap properly
  marginBottom: 12 
},
...
infoValue: { 
  ...
  flexWrap: 'wrap',     // wrap text to new line if too long
},
