...
const HistoryScreen = () => {
    ...
    const authUnsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        const historyUnsubscribe = onSnapshot(historyQuery, (snapshot) => {
          const history: ScanHistoryItem[] = snapshot.docs.map((docSnap) => {
            const data = docSnap.data();
            return {
              id: docSnap.id,
              itemName: data.itemName,
              itemType: data.itemType,
              recycledAt: data.recycledAt,
              co2Saved: data.co2Saved || 0,
              pointsEarned: data.pointsEarned || 0,
              scanResultId:
                typeof data.scanResultId === 'object' && 'id' in data.scanResultId
                  ? data.scanResultId.id
                  : data.scanResultId || '',
            } as ScanHistoryItem;
          });
  ...
  const handleOpenResult = (item: ScanHistoryItem) => {
    ...
    // ✅ Correct way to pass docId via query string
    router.push(`/scan-result?docId=${item.scanResultId}`);
  };
    ...
    <TouchableOpacity style={styles.historyItem} onPress={() => handleOpenResult(item)}>
      ...
      <View style={styles.historyTextContainer}>
        ...
        <View style={styles.badgeRow}>
          {item.co2Saved !== undefined && (
            <Text style={styles.co2Badge}>💨 +{item.co2Saved.toFixed(2)} kg CO₂ Saved</Text>
          )}
          {item.pointsEarned !== undefined && item.pointsEarned > 0 && (
            <Text style={styles.pointsBadge}>🪙 +{item.pointsEarned} Points</Text>
          )}
        </View>
      </View>
